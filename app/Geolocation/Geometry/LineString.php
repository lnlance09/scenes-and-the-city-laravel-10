...
class LineString extends Collection
{
	...
	public function length()
	{
		...
		foreach ($this->getPoints() as $delta => $point) {
			$prevPoint = $this->geometryN($delta);
			if ($prevPoint) {
				$length += sqrt(pow(($prevPoint->getX() - $point->getX()), 2) + pow(($prevPoint->getY() - $point->getY()), 2));
			}
		}
		return $length;
	}
	...
	public function greatCircleLength($radius = 6378137)
	{
		...
		for ($i = 0; $i < $this->numPoints() - 1; $i++) {
			...
			$length += $radius *
				atan2(
					sqrt(
						pow(cos($lat2) * sin($dlon), 2) + pow(cos($lat1) * sin($lat2) - sin($lat1) * cos($lat2) * cos($dlon), 2)
					),
